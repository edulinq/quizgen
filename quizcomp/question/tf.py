...
class TF(quizcomp.question.base.Question, question_type = quizcomp.constants.QUESTION_TYPE_TF):
    ...
    def _validate_answers(self):
        if (isinstance(self.answers, bool)):
            ...
            raise quizcomp.common.QuizValidationError(f"'answers' value must be a boolean, found '{self.answers}' ({type(self.answers)}).")

        self._validate_self_answer_list()

        if (len(self.answers) != 2):
            raise quizcomp.common.QuizValidationError("Expecting exactly two answers, found %d." % (len(self.answers)))

        labels = list(sorted([answer.text for answer in self.answers]))

        expected = ['False', 'True']
        if (labels != expected):
            raise quizcomp.common.QuizValidationError("T/F labels (text) not as expected. Expected: '%s', Actual: '%s'." % (expected, labels))
