...
class Project(quizgen.util.serial.JSONSerializer):
    """
    ...
    def __init__(self,
        ...
        try:
            ...
            raise quizgen.common.QuizValidationError("Error while validating project '%s' ('%s')." % (self._base_dir, self.base_dir)) from ex

    def _validate(self, **kwargs):
        if ((self._base_dir is None) or (self._base_dir == "")):
            raise quizgen.common.QuizValidationError("Base directory cannot be empty.")

        if (not os.path.isdir(self._base_dir)):
            raise quizgen.common.QuizValidationError("Base directory '%s' does not exist or is not a directory." % (self._base_dir))
    ...
    def find_resources(self):
        ...
        quizzes = sorted(glob.glob(os.path.join(self._base_dir, '**', quizgen.constants.QUIZ_FILENAME), recursive = True))
        questions = sorted(glob.glob(os.path.join(self._base_dir, '**', quizgen.constants.QUESTION_FILENAME), recursive = True))
    ...
    def load_resources(self, **kwargs):
        ...
        quizzes = [(path, quizgen.quiz.Quiz.from_path(path, **kwargs)) for path in quiz_paths]
        questions = [(path, quizgen.question.base.Question.from_path(path, **kwargs)) for path in question_paths]
    ...
    def save(self, out_dir = None):
        ...
        if (os.path.isfile(out_dir) and os.path.samefile(out_dir, self._base_dir)):
            self.to_path(os.path.join(self._base_dir, quizgen.constants.PROJECT_FILENAME))
            return

        quizgen.util.dirent.copy_dirent(self._base_dir, out_dir,
                symlinks = True, dirs_exist_ok = True)
        self.to_path(os.path.join(out_dir, quizgen.constants.PROJECT_FILENAME))

    @classmethod
    def from_path(cls, path, **kwargs):
        # If we are looking at a dir, assume the project file is directly inside.
        if (os.path.isdir(path)):
            path = os.path.join(path, quizgen.constants.PROJECT_FILENAME)
