...
class JSONTemplateConverter(quizgen.converter.template.TemplateConverter):
    def __init__(self,
            format = quizgen.constants.FORMAT_JSON, template_dir = DEFAULT_TEMPLATE_DIR,
            **kwargs):
    ...
    def _clean_answers(self, answers, question_type):
        ...
        if (question_type == quizgen.constants.QUESTION_TYPE_MATCHING):
            return self._clean_answers_matching(answers)

        if (question_type == quizgen.constants.QUESTION_TYPE_NUMERICAL):
            return [raw_answer.to_pod() for raw_answer in answers['raw_answers']]

        if (question_type == quizgen.constants.QUESTION_TYPE_FIMB):
            result = []
    ...
    def _clean_answers_list(self, answers, question_type):
        for i in range(len(answers)):
            ...
            if (question_type == quizgen.constants.QUESTION_TYPE_MDD):
